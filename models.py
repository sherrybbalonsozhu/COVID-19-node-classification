...
class GAT(torch.nn.Module):
    def __init__(self):
        ...
        self.gat1 = GATConv(nHiddenUnits*nHeads, out_channels=nHiddenUnits,
                            heads=nHeads, concat=True, negative_slope=alpha,
                            dropout=dropout, bias=True)
        # self.gat1 = GATConv(d.num_node_features, out_channels=nHiddenUnits,
        #                     heads=nHeads, concat=True, negative_slope=alpha,
        ...
        #                     heads=nHeads, concat=False, negative_slope=alpha,
        #                     dropout=dropout, bias=True)
        self.gat2 = GATConv(nHiddenUnits*nHeads, nHiddenUnits,
                            heads=nHeads, concat=True, negative_slope=alpha,
                            dropout=dropout, bias=True)
        self.linear1=torch.nn.Linear(d.num_node_features,nHiddenUnits*nHeads)
        self.linear=torch.nn.Linear(nHiddenUnits*nHeads,d.y.unique().size()[0])

    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        inputs = self.linear1(x)

        x = self.gat1(inputs, edge_index)
        x = F.elu(x)
        x = self.gat2(x, edge_index)
        x=F.elu(x)+inputs
...
class GAT_transformer_averaged(torch.nn.Module):
    def __init__(self):
        ...
        self.gat2 = GATConv(nHeads*nHiddenUnits, out_channels=nHiddenUnits,
                            heads=nHeads, concat=True, negative_slope=alpha,
                            dropout=dropout, bias=True)
        print("edge feature size:", d.edge_attr.shape[1])
        self.transformer = SelfAttention_averaged(d.edge_attr.shape[1])
        self.linear = nn.Linear(72, d.y.unique().size()[0])
        # self.linear = nn.Linear(64, d.y.unique().size()[0])
